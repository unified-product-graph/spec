 ...
 *
 * The base schema is generated from the domain property interfaces in
 * `src/properties/domains/*.ts` by `scripts/generate-property-registry.ts`.
 * On top of that base this file carries a HAND-MAINTAINED overlay the generator
 * cannot reproduce yet, so running the generator over this file is LOSSY. Do NOT
 * blindly regenerate. What a naive regen would strip:
 *   - the `modifier` provenance surface (`derived` / `snapshot` / `volatile`,
 *     property-fit audit 2026-06-16): the interface field below plus ~113
 *     per-property annotations. The domain interfaces carry no modifier info, so
 *     a plain regen drops every one.
 *   - a handful of descriptions and drop-note comments polished past the
 *     generator's terser output (e.g. `constraint_origin`).
 * To change a property: edit the domain source, regenerate into a scratch copy,
 * port the mechanical change back, and re-apply the overlay by hand. Teaching the
 * generator to read and re-emit modifiers so this file can rejoin
 * `check:generated` is a tracked follow-up (0.17.3 #4, Ro-Laren P2).
 *
 * Base generated: 2026-06-14T09:18:17.518Z
 */
