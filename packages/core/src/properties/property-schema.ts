...
export const UPG_PROPERTY_SCHEMA: Record<string, PropertySchema> = {
  ...
  composition: {
    member_query: { type: 'object', description: 'When present, membership is DERIVED: members are produced by running this query rather than authored by placement.', notes: 'This is the portable statement of what the layer shows. On a composition, `CompositionMember.href` remains the publishing tool\'s own resolved route and stays opaque to everyone else; a member may carry both, and then the href is a fast path while the query is the meaning. A consumer that cannot parse the href can still render the layer, which is the whole reason the declaration is here rather than in a tool-namespaced bag key. A layer with no `member_query` is authored, which is what every composition written before 0.32.0 is. DECLARED ON BOTH HALVES OF THE PAIR since 0.33.0. A layer is query-driven while it is being worked on, not only once it is published, so declaring the query only on the durable composition would make it a fact invented at publish time rather than one recorded.' },
    presentation: { type: 'object', description: 'Advisory rendering intent for the layer as a whole. A consumer may ignore it entirely and still be conformant.' },
