...
export const UPG_PROPERTY_SCHEMA: Record<string, PropertySchema> = {
  ...
  composition: {
    ...
    member_query: { type: 'object', description: 'When present, membership is DERIVED: members are produced by running this query rather than authored by placement.', notes: 'This is the portable statement of what the composition shows. `CompositionMember.href` remains the publishing tool\'s own resolved route and stays opaque to everyone else; a member may carry both, and then the href is a fast path while the query is the meaning. A consumer that cannot parse the href can still render the layer, which is the whole reason the declaration is here rather than in a tool-namespaced bag key. A composition with no `member_query` is authored, which is what every composition written before 0.32.0 is.' },
    presentation: { type: 'object', description: 'Advisory rendering intent for the composition as a whole. A consumer may ignore it entirely and still be conformant.' },
  },
  ...
  product: {
    ...
    key_prefix: { type: 'string', description: 'Prefix for keys minted onto this product\'s nodes (e.g. `"LTN"`, giving `LTN-1`, `LTN-2`, …). A product with no prefix mints no keys. @example "LTN"', notes: 'Pairs with `UPGBaseNode.key`, which holds the minted value. The prefix sits on the product because the key\'s uniqueness scope is the product, across entity types; the namespace a tracker calls a "team". Only the prefix is serialised: the next number is `max(existing) + 1`, derived from the graph, because a counter is a fact about a store rather than about the product.' },
  },
