...
export const UPG_EDGE_MIGRATIONS: Record<string, UPGEdgeMigration[]> = {
  '0.9.9': [
    ...
    { kind: 'rename', from: 'revenue_stream_measured_by_metric_cross_domain', to: 'revenue_stream_measured_by_metric', requires_source_type: 'revenue_stream', requires_target_type: 'metric', reason: 'Duplicate collapse ( T0.1, 0.13.0 Wave 1). Cross-domain-classified shadow of revenue_stream_measured_by_metric (same measured_by/measures verbs); pickCanonicalEdge never returned it (the semantic key wins the pair; revenue_stream_drives_metric serves the cross-domain ask). Verb-preserving collapse onto the semantic canonical.' },
